...
![Release](https://img.shields.io/badge/Release-v0.9.1-4b5563.svg)
...
>
> ```text
> 请运行 scripts/setup-local.sh，只使用仓库示例样本生成 Codex 用量 Dashboard，不读取我的真实 Codex 历史、浏览器 cookie、token、钥匙串或系统凭据，不上传任何数据。最后告诉我 Dashboard 和报告路径。
> ```
>
> 分析自己的本地 Codex 会话：
>
...
>
> ```text
> 帮我安装这个仓库 https://github.com/dososo/BLCaptain-Codex-Probe-CLI。安装完成后先使用仓库示例数据生成 Codex 用量 Dashboard，不读取我的真实 Codex 历史、浏览器 cookie、token、钥匙串或系统凭据，不上传任何数据。
> ```
>
> 也可以手动运行：
>
...
> ```
>
> 无参数时会运行仓库示例数据流程：创建 `.venv`、安装 CLI、初始化账本、dry-run、导入仓库示例样本、生成并打开 Dashboard。
>
> 如果想体验 macOS 状态栏入口：
>
> ```bash
...
> ```
>
> 这是源码本地构建入口。GitHub 开源、CLI 安装和用户本地构建不需要 Apple Developer ID；只有维护者发布给普通用户双击安装的 `.app` / `.dmg` 时，才需要考虑签名、公证和 Gatekeeper 验证。见 [macOS 正式分发、签名与公证](docs/MACOS_RELEASE_DISTRIBUTION.md)。
>
...
当前推荐发布定位：**CLI 是正式开源主入口，macOS 状态栏 App 是可选源码本地构建入口**。因此 GitHub 开源发布不需要 Apple Developer Program；签名、公证只适用于维护者未来分发可双击安装的 Mac App。

v0.9.1 在真实数据源接入、稳定 watcher、项目聚合、周报和阶段级治理之上，继续提供 **原生 macOS 状态栏 App**。它把 CLI 的会话账本、预算预警、数据源可信度和 Dashboard 入口放进系统菜单栏，优先回答这件事：
...
v0.9.1 提供 BLCaptain Codex Probe Bar：一个原生 macOS 菜单栏入口。它不重复实现账本逻辑，而是调用本地 `codex-probe --json`，展示最高消耗会话、token 总量、预算预警、数据源可信度和常用动作。
...
<p align="center">
  <img src="assets/screenshots/macos-menubar-app.png" alt="BLCaptain Codex Probe Bar macOS 状态栏下拉面板真实截图" width="620">
</p>
...
| 能力 | 命令 | 结果 |
|---|---|---|
| 一键本地 setup | `scripts/setup-local.sh` 或 `codex-probe setup --sample` | 安装/初始化、dry-run、报告、Dashboard 一次完成 |
...
| 本地预算预警 | `codex-probe alerts --range 30d` | 基于可配置阈值输出单会话、项目、总账和上下文风险提示 |
...
| `examples/reports/ledger/` | v0.9.1 会话账本、阶段时间线、预警、可信度、任务归因、项目汇总和周报示例报告 |
...
│   ├── INSTALL.md                     # 轻量安装方式
│   ├── MACOS_MENUBAR_APP.md          # macOS 状态栏 App 源码本地构建
...
- 能跑通 `setup --sample`、`watch once/start/status/logs/stop/status-page` 和 `delete --watcher --yes`。
- 能输出会话排行、单会话报告、总账报告、阶段级时间线、本地预算预警、任务类型归因、数据源可信度、隐私审计报告、本地 HTML Dashboard 和 watcher 状态页。
- 能输出项目级汇总报告和 Codex 周报。
- macOS 状态栏 App 可以 `swift build`，并可通过脚本安装到 `~/Applications/CodexProbeBar.app`。
