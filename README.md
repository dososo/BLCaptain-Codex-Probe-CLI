...
v0.5.0 的核心能力从「可导入样本的账本」继续升级为 **真实数据源接入 + 稳定 watcher**。它优先回答这件事：
...
| `examples/reports/ledger/` | v0.5.0 会话账本示例报告 |
...
- 能跑通 `watch once/start/status/logs/stop` 和 `delete --watcher --yes`。
