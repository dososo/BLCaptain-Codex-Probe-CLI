...
在 Codex 桌面版 App 里，一句话就可以让 Codex 完成本地安装、导入示例数据、生成 doctor 汇总报告、体检 Skill 和删除验证。
...
| **分析** | Token / credits / 上下文余量、5 小时额度、7 天额度、预算风险、降配建议、停止建议 |
| **体检** | AI 味、插件风险、缺失验收、缺失隐私边界、敏感信息脱敏 |
| **输出** | 决策卡片、Markdown 报告、JSON 命令输出、可复核 evidence_id |
...
- `reports/doctor/doctor-report.md`：总览、隐私边界、建议动作。
- `reports/doctor/usage-report.md`：任务级用量报告和决策卡片。
- `reports/doctor/skill-lint-report.md`：Skill / 输出质量体检。

`doctor` 只读取你显式传入的 `--status`、`--manual-json` 或 `--skill` 文件，不会扫描系统目录。
...
2. 用 examples/status-codex-desktop.txt 和 examples/risky-skill.md 跑一次 doctor。
3. 报告放到 reports/doctor/。
4. 最后告诉我 doctor 报告路径、主要风险、建议继续、降配还是停止。
...
- 上下文剩余、已用 / 上限 token、5 小时额度、7 天额度和重置时间。
- 顶部决策卡片：建议动作、为什么贵、怎么降配、什么时候该停。
- 是否超过预算或接近预算。
...
安装依赖，用 examples/status-codex-desktop.txt 和 examples/risky-skill.md 跑 doctor，
生成 reports/doctor/doctor-report.md、usage-report.md 和 skill-lint-report.md。
完成后只给我报告路径、关键风险、建议继续、降配还是停止，以及验证结果。
```
...
2. 一份带决策卡片的任务级用量自查报告。
3. 一份 Skill / 输出质量体检报告。
4. 一组 JSON 命令输出，便于 Agent 或脚本继续处理。
5. 删除后的 `NO_USAGE_DATA` 可验证结果。
...
| `examples/reports/doctor/doctor-report.md` | `doctor` 一键体检汇总报告 |
...
- 能用 `doctor` 一条命令生成汇总报告、用量报告和 Skill 体检报告。
- 能生成任务级用量报告。
- 能生成 Skill / 输出质量体检报告。
- 能删除本地业务数据。
