...
v0.4.0 的核心能力从单次 `/status` 用量体检升级为 **本地 Codex 会话级 Token 账本**。它优先回答这件事：
...
| 导入官方导出 | `codex-probe ledger import --official-export <file>` | exact 置信度会话归因 |
...
- `exact`：用户显式提供的官方导出，且包含会话 ID 和 token。
- `high`：本地结构化快照能明确归因到单会话。
...
| `examples/reports/ledger/` | v0.4.0 会话账本示例报告 |
...
2. 使用 examples/ledger-samples/official-export.csv 和 examples/ledger-samples/snapshot-delta.json。
...
- `codex-probe --version` 返回 `0.4.0`。
- 能导入官方导出和快照样本。
- 能输出会话排行、单会话报告、总账报告、隐私审计报告和本地 HTML Dashboard。
...
- 扩充真实脱敏样本库，提升不同 `/status` 和导出格式的兼容性。
- 增加项目级聚合和周报。
...
A：不能。P0 支持用户显式提供的官方导出或本地快照，不读取登录态、浏览器 cookie 或官方后台隐藏数据。

**Q：它能精确知道每条真实会话的 token 吗？**

A：只有数据源本身提供会话 ID 和 token 时才是 `exact`。快照和全局额度变化会标注为 `high`、`medium` 或 `low`。

**Q：它能保证省钱吗？**

A：不能。它只能定位消耗、解释风险、给出降配和停止建议，最终仍要由你判断。
